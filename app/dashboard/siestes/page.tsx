...
export default function Siestes() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('siestes').insert([
      { enfant, heure_debut: heureDebut, heure_fin: heureFin, note, date: new Date().toISOString() }
    ])
    if (!error) {
      setSucces(true)
      setEnfant('')
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="px-6 py-10 max-w-lg mx-auto">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">😴 Enregistrer une sieste</h1>
        {succes && <div className="bg-green-100 text-green-700 px-4 py-3 rounded-lg mb-4">Sieste enregistrée ✅</div>}
